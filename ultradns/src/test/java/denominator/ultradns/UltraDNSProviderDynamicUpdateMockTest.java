...
public class UltraDNSProviderDynamicUpdateMockTest {
  ...
  public void dynamicAccountIdUpdatesOnEndpoint() throws IOException, InterruptedException {
    ...
    try {
      ...
      assertEquals(new String(server.takeRequest().getBody()),
                   getZonesOfAccount.replace("AAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBBB"));
    } finally {
      server.shutdown();
    }
  }
  ...
  public void dynamicEndpointUpdates() throws IOException, InterruptedException {
    MockWebServer server = new MockWebServer();
    server.enqueue(
        new MockResponse().setResponseCode(200).setBody(getResourceRecordsOfZoneResponseAbsent));
    server.enqueue(
        new MockResponse().setResponseCode(200).setBody(getResourceRecordsOfZoneResponseAbsent));
    server.play();
    ...
    try {
      ...
      assertNull(api.basicRecordSetsInZone("denominator.io.")
                     .getByNameAndType("www.denominator.io.", "A"));
      dynamicUrl.set(new URL(mockUrl, updatedPath));
      assertNull(api.basicRecordSetsInZone("denominator.io.")
                     .getByNameAndType("www.denominator.io.", "A"));
