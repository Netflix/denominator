...
public abstract class BaseReadOnlyLiveTest extends BaseProviderLiveTest {
  ...
  protected void checkGetByNameTypeAndQualifierConsistent(Zone zone, ResourceRecordSet<?> rrs) {
    ResourceRecordSet<?>
        ...
        allApi(zone).getByNameTypeAndQualifier(rrs.name(), rrs.type(),
                                               rrs.qualifier());
    assertTrue(byNameTypeAndQualifier != null,
               "could not lookup by name, type, and qualifier: " + rrs);
  ...
  protected void checkListByNameAndTypeConsistent(Zone zone, ResourceRecordSet<?> rrs) {
    ...
    assertTrue(byNameAndType.contains(rrs),
               rrs + " not found in list by name and type: " + byNameAndType);
  }
