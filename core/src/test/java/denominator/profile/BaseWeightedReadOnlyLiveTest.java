...
public abstract class BaseWeightedReadOnlyLiveTest extends BaseProviderLiveTest {
  ...
  static void checkWeightedRRS(ResourceRecordSet<?> weightedRRS) {
    ...
    assertFalse(weightedRRS.records().isEmpty(),
                "Values absent on ResourceRecordSet: " + weightedRRS);
  }
  ...
  private void testListRRSs() {
    ...
    for (Zone zone : zones()) {
      for (ResourceRecordSet<?> weightedRRS : weightedApi(zone)) {
        assertNotNull(weightedRRS.qualifier(),
                      "Weighted record sets should include a qualifier: " + weightedRRS);
        checkWeightedRRS(weightedRRS);

        Weighted weighted = weightedRRS.weighted();
        assertTrue(weightedApi(zone).supportedWeights().contains(weighted.weight()));
        assertTrue(
            manager.provider().profileToRecordTypes().get("weighted").contains(weightedRRS.type()));
        ...
        weightedRecordCounts.getUnchecked(weightedRRS.weighted())
            .addAndGet(weightedRRS.records().size());

        Iterator<ResourceRecordSet<?>>
            byNameAndType =
            weightedApi(zone).iterateByNameAndType(weightedRRS.name(), weightedRRS.type());
        ...
        assertNotNull(byNameTypeAndQualifier,
                      "could not lookup by name, type, and qualifier: " + weightedRRS);
  ...
  private void testListByName() {
    ...
    for (Zone zone : zones()) {
      ...
      List<ResourceRecordSet<?>>
          fromApi =
          Lists.newArrayList(weightedApi(zone).iterateByName(name));
      assertEquals(usingToString().immutableSortedCopy(fromApi),
                   usingToString().immutableSortedCopy(withName));
      break;
    }
  }
  ...
  private void testGetByNameTypeAndQualifierWhenAbsent() {
    ...
    for (Zone zone : zones()) {
      assertNull(
          weightedApi(zone).getByNameTypeAndQualifier("ARGHH." + zone.name(), "TXT", "Mars"));
      break;
    }
  }
