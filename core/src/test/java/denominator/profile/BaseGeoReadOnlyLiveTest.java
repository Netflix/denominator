    ...
    for (Zone zone : zones()) {
      for (ResourceRecordSet<?> geoRRS : geoApi(zone)) {
        ...
        assertTrue(Iterators.elementsEqual(
            geoApi(zone).iterateByNameAndType(geoRRS.name(), geoRRS.type()), byNameAndType));

        ResourceRecordSet<?>
            byNameTypeAndQualifier =
            geoApi(zone).getByNameTypeAndQualifier(geoRRS.name(),
                                                   geoRRS.type(), geoRRS.qualifier());
        assertNotNull(byNameTypeAndQualifier,
                      "could not lookup by name, type, and qualifier: " + geoRRS);
