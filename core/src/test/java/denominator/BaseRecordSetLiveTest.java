...
public abstract class BaseRecordSetLiveTest extends BaseProviderLiveTest {
  ...
  private void testListRRSs() {
    ...
    for (Zone zone : zones()) {
      for (ResourceRecordSet<?> rrs : rrsApi(zone)) {
        checkRRS(rrs);
        assertTrue(rrs.qualifier() == null,
                   "Only basic record sets should be returned. found: " + rrs);
  ...
  private void deleteRRS(ResourceRecordSet<?> recordSet) {
    ...
    String
        failureMessage =
        format("recordset(%s, %s) still exists in %s", recordSet.name(), recordSet.type(), zone);
    assertTrue(rrsApi(zone).getByNameAndType(recordSet.name(), recordSet.type()) == null,
               failureMessage);
    assertFalse(any(rrsApi(zone).iterator(),
                    predicate(nameAndTypeEqualTo(recordSet.name(), recordSet.type()))),
                failureMessage);
