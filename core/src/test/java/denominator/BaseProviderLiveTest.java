...
public abstract class BaseProviderLiveTest {
  ...
  protected Map<String, ResourceRecordSet<?>> stockRRSets() {
    ...
    Builder<String, ResourceRecordSet<?>>
        builder =
        ImmutableMap.<String, ResourceRecordSet<?>>builder();
    builder.put(
        "AAAA",
    ...
    builder.put("A", a("ipv4-" + recordSuffix,
  ...
  protected void skipIfRRSetExists(Zone zone, String name, String type, String qualifier) {
    if (any(rrsApi(zone).iterator(),
            predicate(nameTypeAndQualifierEqualTo(name, type, qualifier)))) {
      throw new SkipException(
          format("recordset(%s, %s, %s) already exists in %s", name, type, qualifier, zone));
    }
  }
  ...
  protected void assertPresent(ResourceRecordSet<?> rrs, Zone zone, String name, String type,
    ...
    if (rrs == null) {
      throw new AssertionError(
          format("recordset(%s, %s%s) not present in %s; rrsets: %s", name, type,
                 qualifier != null ? ", " + qualifier : "", zone,
                 Iterators.toString(rrsApi(zone).iterator())));
    }
  }
