...
class VrsnMdnsErrorDecoder implements ErrorDecoder {
	...
	static {
		MDNS_ERROR_TO_INT_CODE_MAP.put("ERROR_MISSING_INVALID_INPUT",
				new Integer(1));
		MDNS_ERROR_TO_INT_CODE_MAP.put("ERROR_RULE_VALIDATION", new Integer(2));
		MDNS_ERROR_TO_INT_CODE_MAP.put("ERROR_OPERATION_FAILURE",
				new Integer(3));
	...
	public Exception decode(String methodKey, Response response) {
		try {
			...
			VrsnMdnsError error = VrsnMdnsError.class.cast(decoder.decode(
					response, VrsnMdnsError.class));
			if (error == null)
	...
	static class VrsnMdnsError extends DefaultHandler implements
			ContentHandlerWithResult<VrsnMdnsError> {
		@Inject
		VrsnMdnsError() {
		}
		...
		public void startElement(String uri, String localName, String qName,
			...
			if (qName.endsWith(":reason")) {
				try {
					Integer tempCode = MDNS_ERROR_TO_INT_CODE_MAP
							.get(attributes.getValue("code"));
	...
	static Response bufferResponse(Response response) throws IOException {
		...
		return Response.create(response.status(), response.reason(),
				response.headers(), body, UTF_8);
	}
