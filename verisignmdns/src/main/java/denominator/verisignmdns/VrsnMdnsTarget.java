...
class VrsnMdnsTarget implements Target<VrsnMdns> {
    ...
    @Inject
    VrsnMdnsTarget(Provider provider, javax.inject.Provider<Credentials> credentials) {
    ...
    public Request apply(RequestTemplate input) {
        input.insert(0, url());
        input.body(format(SOAP_TEMPLATE, VrsnUtils.getUsername(credentials), VrsnUtils.getPassword(credentials),
    ...
    static final String SOAP_TEMPLATE = "" //
            ...
            + "<S:Header>" + "<urn1:authInfo>" + "<urn1:userToken>"
            + "<urn1:userName>%s</urn1:userName>"
            + "<urn1:password>%s</urn1:password>" + "</urn1:userToken>" + "</urn1:authInfo>"
            + "</S:Header>"
