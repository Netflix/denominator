...
class EncodeChanges implements Encoder {
    ...
    public void encode(Object object, RequestTemplate template) throws EncodeException {
        ...
        for (ActionOnResourceRecordSet change : actions)
            b.append("<Change>").append("<Action>").append(change.action).append("</Action>").append(apply(change.rrs))
                    .append("</Change>");
        b.append("</Changes>");
    ...
    static String apply(ResourceRecordSet<?> rrs) {
        ...
        if (rrs.records().size() == 1 && rrs.records().get(0).containsKey("HostedZoneId")) {
            ...
            for (String attribute : new String[] { "HostedZoneId", "DNSName" }) {
                ...
                builder.append(checkNotNull(aliasTarget.get(attribute), "missing %s in alias target %s", attribute, rrs));
                builder.append("</").append(attribute).append('>');
            }
